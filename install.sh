...
# ---------------------------------------------------------------------------
# STEP 4: Mosquitto Setup  (cloud mode only)
# ---------------------------------------------------------------------------
if [ "$DEPLOYMENT_MODE" != "cloud" ]; then
    : # skip for air-gapped
elif step_done "mosquitto_setup"; then
    ...
    if [ "$INSTALL_LOCAL_NGINX" = "false" ]; then
        # External proxy path: mosquitto here listens plain on the LAN, and
        # the external proxy (nginx stream, HAProxy, etc.) terminates TLS on
        # MQTT_EXTERNAL_PORT and forwards plain to this host's :1883.
        # Do NOT try to copy Let's Encrypt certs (we never ran certbot here)
        # and do NOT bind a TLS listener that would just fail.
        echo "Writing $MOSQUITTO_CONF (plain LAN, TLS handled by external proxy) ..."
        sudo tee "$MOSQUITTO_CONF" > /dev/null << EOF
# LocalDroid MQTT Configuration — generated by install.sh on $(date)
#
# External-proxy mode: mosquitto listens plain on the LAN. Devices reach it
# via $MQTT_EXTERNAL_HOST:$MQTT_EXTERNAL_PORT through an upstream TLS
# terminator (e.g. nginx stream {} block on a separate host). The Go server
# connects on localhost:1883.
...
EOF
        ...
        if [ -n "$MQTT_USERNAME" ] && [ -n "$MQTT_PASSWORD" ]; then
            ...
            sudo mosquitto_passwd -c -b "$PASSWD_FILE" "$MQTT_USERNAME" "$MQTT_PASSWORD" 2>/dev/null
...
EOF
            echo -e "  ${GREEN}MQTT password file created${NC}"
        fi
        ...
        if systemctl is-active --quiet mosquitto; then
            ...
            echo -e "${GREEN}Mosquitto running and listening on 0.0.0.0:1883 (plain, LAN-reachable at $LAN_IP:1883).${NC}"
            echo -e "${CYAN}Configure your external TLS terminator to proxy ${MQTT_EXTERNAL_PORT}/tcp (TLS)${NC}"
            echo -e "${CYAN}  → ${LAN_IP}:1883 (plain).${NC}"
        ...
        echo "Writing $MOSQUITTO_CONF ..."
        sudo tee "$MOSQUITTO_CONF" > /dev/null << EOF
# LocalDroid MQTT Configuration — generated by install.sh on $(date)

# Local listener: Go server connects here (no TLS needed on loopback)
listener 1883 127.0.0.1
allow_anonymous true

# Public TLS listener: Android/Windows devices connect here
listener $MQTT_EXTERNAL_PORT
cafile   $MQTT_CERT_DIR/chain.pem
...
EOF
        ...
        # Cert renewal hook — auto-copies renewed certs and restarts Mosquitto
...
HOOK
        ...
        sudo systemctl restart mosquitto
        if systemctl is-active --quiet mosquitto; then
            echo -e "${GREEN}Mosquitto running and listening on 127.0.0.1:1883 and 0.0.0.0:$MQTT_EXTERNAL_PORT${NC}"
            mark_done "mosquitto_setup"
        else
            echo -e "${RED}Mosquitto failed to start. Check: sudo journalctl -u mosquitto -n 20${NC}"
            exit 1
